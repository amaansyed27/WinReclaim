use crate::domain::{AiStatus, Finding};
use anyhow::Result;
use uuid::Uuid;
...
#[derive(Debug)]
pub(crate) struct IntentConstraints {
...
pub(crate) fn build_constraints(
    prompt: &str,
    candidates: &[&Finding],
) -> Result<(String, IntentConstraints)> {
...
mod tests {
    ...
    fn parses_compact_and_spaced_targets() {
        ...
        assert_eq!(
            parse_target_bytes("free 1.5TB"),
            Some((1.5 * 1024_f64.powi(4)) as u64)
        );
    }
    ...
    fn review_first_requires_explicit_phrase() {
        ...
        assert!(explicitly_accepts_review_first(
            "include review-first actions"
        ));
    }
